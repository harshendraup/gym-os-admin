...
// ─── Super Admin View (gym picker + members) ──────────────────────────────────
...
function SuperAdminMembersView() {
  ...
  const { data: gymsData, isLoading: gymsLoading } = useQuery({
    ...
    queryFn: () => getPaginated<{ id: string; name: string; city: string | null }>('/admin/gyms', { perPage: 100 }),
  })

  const gyms = gymsData?.data ?? []

  const { data, isLoading } = useQuery({
    queryKey: ['members', selectedGymId, { search: debouncedSearch, status, page }],
    queryFn: () => membersApi.list(selectedGymId, {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: '#64748B' }}>
            {selectedGymId ? `${data?.meta?.total ?? 0} members in selected gym` : 'Select a gym to view members'}
          </p>
        </div>
        {selectedGymId && (
          <button
      ...
      >
        ...
        <span className="text-sm font-medium text-slate-600 flex-shrink-0">Gym</span>
        <select
          value={selectedGymId}
          onChange={(e) => { setSelectedGymId(e.target.value); setPage(1) }}
          ...
          style={{
            ...
            color: selectedGymId ? '#1e293b' : '#64748B',
          }}
      ...
      {/* Filters (shown only once a gym is selected) */}
      {selectedGymId && (
      ...
      <DataTable
        columns={columns}
        data={data?.data ?? []}
        isLoading={isLoading && !!selectedGymId}
        pagination={{
          page,
          pageCount: data?.meta?.lastPage ?? 1,
          onPageChange: setPage,
        }}
        emptyMessage={selectedGymId ? 'No members found.' : 'Select a gym above to view its members.'}
      />
